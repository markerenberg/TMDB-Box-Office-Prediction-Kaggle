...
train_target_encode <- function(df,feature){  
  ...
  ext_train <- unique(ext_train[,c(1,ncol(ext_train))])
...
for(x in 1:10){
  ...
  tedf = actor_telist[x][[1]][[2]]
  actors_df = left_join(actors_df,tedf,by=feature)
}
...
# Store indices in placeholder variables
actor1index = which(names(actors_df) == 'actor1')
actor10index = which(names(actors_df) == 'actor10')

topactorslist = lapply(actors_df[,actor1index:actor10index],function(actors){
  unlist(lapply(actors, as.character), use.names = FALSE)
})
...
################# Grid Search #################
target <- df$revenue
dtrain <- xgb.DMatrix(data = as.matrix(sapply(df[,-1],as.numeric)), label = df$revenue)


searchGridSubCol <- expand.grid(subsample = c(0.5, 0.75, 1), 
                                colsample_bytree = c(0.4, 0.6, .8, 1),
                                max_depth = c(4,6,8,10),
                                min_child = seq(0.2,0.5,1), 
                                eta = c(0.25,.5,.75,1),
                                ntrees = c(100)
)
...
system.time(
  rmseErrorsHyperparameters <- apply(searchGridSubCol, 1, function(parameterList){
    ...
    xgboostModelCV <- xgb.cv(data =  dtrain, nrounds = ntrees, nfold = 5, showsd = TRUE, 
                             ...
                             "max.depth" = currentDepth, "eta" = currentEta,                               
...
output <- output[order(output$TestRMSE),]
output[1:10,]
